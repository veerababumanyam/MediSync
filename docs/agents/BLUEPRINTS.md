...
| [E-01](#e-01-language-detection--routing-agent) | Language Detection & Routing Agent | Language & i18n | 2 | L1 | Read-only |
...
## E-01 — Language Detection & Routing Agent

### Purpose
Acts as the **mandatory pre-processing gate** for all chat queries. Detects query language, resolves the user's active response locale, and injects locale context into all downstream Genkit flows so every agent responds in the user's preferred language.

> Full detailed spec: [specs/e-01-language-detection-routing.md](./specs/e-01-language-detection-routing.md)

### Inputs
| Field | Type | Source |
|-------|------|--------|
| `raw_query` | string | Chat interface |
| `user_locale_pref` | string | JWT claim `locale` |
| `ai_response_lang` | string | User preferences (`en` / `ar` / `auto`) |
...
| Field | Type | Description |
|-------|------|-------------|
| `detected_language` | string | `en` or `ar` |
| `response_locale` | string | Locale to use for AI response |
| `normalised_query` | string | Query ready for downstream (English intent if translated) |
| `original_query` | string | Preserved for display and audit |
| `requires_translation` | bool | Whether E-02 must translate the query |
| `confidence` | float64 | Language detection confidence |

### Tool Chain
```
User Query
  → Unicode Heuristic (Go built-in, < 2ms)
  → [optional] Gemini Flash 1.5 fast-classify (only if confidence < 0.75)
  → Resolve response_locale (from ai_response_lang pref)
  → Flag requires_translation
  → [if ar query] E-02 Arabic→Intent translation
  → Inject locale into Genkit FlowContext
...
### OSS Components
- **Go `unicode` package** — Arabic range detection (U+0600–U+06FF)
- **`golang.org/x/text/language`** — BCP-47 language matching
- **Genkit** — Flow context propagation
- **Gemini Flash 1.5** — Low-cost classification fallback
- **OpenTelemetry** — Trace propagation

### System Prompt (Fast Classification)
```
You are a language detection classifier.
Output EXACTLY ONE token: "en" or "ar". No explanation, no punctuation.
Input: {{.raw_query}}
```

### Guardrails
- Unsupported locale detected → default to `user_locale_pref`; log `UNSUPPORTED_LOCALE` warning.
- E-01 failure → safe fallback: pass `raw_query` unmodified, use `user_locale_pref` as locale. **Never block user query.**
- SQL identifiers are never translated — E-02 is instructed to preserve all technical names verbatim.
- Total E-01 latency target: < 5ms (Unicode path), < 600ms (LLM fallback path).

### HITL Gate
None. E-01 is a transparent pre-processing step with silent fallback. Never blocks.

### Evaluation Criteria
| Metric | Target |
|--------|--------|
| Language detection accuracy | > 98% |
| False Arabic detection (numbers-only queries) | < 1% |
| E-01 latency p95 (Unicode path) | < 5ms |
| E-01 latency p95 (LLM fallback) | < 600ms |
| Downstream response in correct locale | > 97% |
