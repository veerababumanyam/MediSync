      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {suggestions.map((suggestion, index) => (
          <button
            ...
            className="liquid-glass-button-prominent group flex items-center gap-3 px-4 py-3 min-h-[44px] text-start liquid-glass-hover-lift liquid-focus-gold"
            role="listitem"
          ...
          >
            <div className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{
              background: 'var(--gradient-brand-alpha, linear-gradient(135deg, rgba(39,80,168,0.1) 0%, rgba(24,146,157,0.1) 100%))'
            }}>
...
const SuggestionIcon: React.FC<{ index: number }> = ({ index }) => {
  ...
  const icons = [
    ...
    // Chart/Departments
    <svg key="1" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="var(--brand-secondary)" strokeWidth={2} aria-hidden="true">
    ...
    // Inventory/Box
    <svg key="3" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="var(--brand-secondary)" strokeWidth={2} aria-hidden="true">
